...
def test_build_silver_movie_metrics(temp_project):
    for prov in ["boxofficemetrics", "criticagg", "audiencepulse"]:
        write_audit(
            provider=prov, batch_id="b1", level="batch", complete=True, status="ok"
        )
    ...
    ).to_csv(bd / "consolidated.csv", index=False)
    ...
    ).to_csv(bi / "consolidated.csv", index=False)
    ...
    ).to_csv(bf / "consolidated.csv", index=False)
    ...
    ).to_csv(cr / "consolidated.csv", index=False)
        ...
        [
            {
                ...
                "title": "A",
                "year": 2001,
                "audience_average_score": 9.0,
                "audience_votes": 50,
            }
        ]
    ).to_csv(ap / "consolidated.csv", index=False)
    contracts = {
